...
from core.database import (
    init_db, generate_synthetic_data,
    set_business_meta, get_business_meta,
)
...
def get_future_seeded_conflict_info() -> Optional[dict]:
    ...
    try:
        now = datetime.now()
        # Ana's booking is on day+3, 4:00 PM
        row = conn.execute(
            """SELECT o.id, o.scheduled_at, o.provider_id,
               ...
               WHERE o.status = 'confirmed'
                 AND o.scheduled_at > ?
                 AND (o.notes LIKE '%Ana-conflict-seed%' OR u.full_name = 'Ana Thompson')
...
def scenario_conflicts(bname: str, btype: str, chunks: list, all_user_ids: list[int]) -> TestResults:
    ...
    provider = get_provider_by_name("Peter") or get_provider_by_name("Ana") or (
  ...
  python smoke_test.py --keep-db               # keep DB files after run
    ...
    parser.add_argument(
        ...
        choices=["restaurant", "dry_cleaner", "appointment", "info",
                 "mixed", "conflict", "db_integrity", "loyalty"],
    ...
    salon_file  = os.path.join(sample_dir, "photography.md")   # extra business
    ...
    # Suite 1 — Pizza restaurant (order scenarios)
    pizza_scenarios = scenarios or ["restaurant", "info", "mixed", "loyalty", "db_integrity"]
    r1 = run_full_suite(pizza_file, "Mario Pizza", pizza_scenarios)
    ...
    # Suite 3 — Optional extra business (photography / salon)
    if not scenarios and os.path.exists(salon_file):
        r3 = run_full_suite(salon_file, "Photography Studio", ["appointment", "info", "conflict"])
