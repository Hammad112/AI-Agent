...
def get_customer_context(user_id: int, session_id: str = "", business_name: str = "") -> dict:
    """Gather all customer data for context window, filtered by business."""
    db_name = os.getenv("DB_NAME", "business_agent.db")
    conn = sqlite3.connect(db_name)
    conn.row_factory = sqlite3.Row
    try:
        ...
        orders = conn.execute(
            """SELECT o.id, o.status, o.order_type, o.scheduled_at, o.total_price,
               ...
               WHERE o.user_id = ? AND s.business_name = ? ORDER BY o.scheduled_at DESC LIMIT 8""",
        ...
        events = conn.execute(
            """SELECT e.*, s.name as service_name FROM calendar_events e
               ...
               WHERE e.user_id = ? AND s.business_name = ? AND e.status != 'cancelled' 
        ...
        if session_id:
            ...
            cart_items = conn.execute(
                """SELECT c.*, s.name as service_name FROM cart c
                   LEFT JOIN services s ON c.service_id = s.id
                   WHERE c.user_id = ? AND c.business_name = ?""",
                (user_id, business_name),
            ).fetchall()

        # Recent complaints
        complaints = conn.execute(
            """SELECT c.complaint_type, c.description, c.status, c.created_at FROM complaints c
        ...
        fav = conn.execute(
            """SELECT s.name, COUNT(o.id) as cnt FROM orders o
               ...
               WHERE o.user_id = ? AND s.business_name = ? GROUP BY s.name ORDER BY cnt DESC LIMIT 1""",
            (user_id, business_name),
        ).fetchone()
